...
async fn run_app(m: ArgMatches<'static>, configuration: Config) -> std::result::Result<(), ()> {
    ...
    // Setup logging
    setup_logging(m.value_of("VERBOSITY").unwrap(), m.is_present("DAEMONIZE"));
    ...
    let peers: Vec<_> = match m
        ...
        None if !dynamic => elog("Failed to initialise ratmand: missing peers data!", 2),
        None => vec![],
    };
        ...
        .await
        {
            Ok(tcp) => {
                ...
                if m.is_present("USE_UPNP") {
                    if let Err(e) = upnp::open_port(tcp.port()) {
                        error!("UPNP setup failed: {}", e);
                    }
                }

                let peers: Vec<_> = peers.iter().map(|s| s.as_str()).collect();
                match attach_peers(&tcp, peers).await {
                    Ok(()) => tcp,
                    Err(e) => elog(format!("failed to parse peer data: {}", e), 1),
                }
            }
            Err(e) => elog(format!("failed to initialise TCP endpoint: {}", e), 1),
        };
    ...
    if !m.is_present("NO_WEBUI") {
        match web::start(r.clone(), "127.0.0.1", 8090).await {
            Ok(_) => {}
    ...
    {
        Ok(addr) => addr,
        Err(e) => elog(format!("Failed to parse API_BIND address: {}", e), 2),
    };
    if let Err(e) = run(r, api_bind).await {
...
fn sysv_daemonize_app(
    ...
    let pid_file = match pidfile::PidFile::new(&pid_filepath).and_then(|f| f.write_pid()) {
        Ok(v) => v,
        Err(err) => {
            let err = err as i32;
