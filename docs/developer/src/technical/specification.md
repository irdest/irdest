...
A space (or namespace) is a group address, shared across many devices or applications.  They can be _long-term_ (baked into the source of an application) or _ephemeral_ (created programatically) and allow messages to be sent to multiple recipients while avoiding having to encode/ encrypt a payload multiple times.

In isolation a regular address and space address are _indistinguishable_ from each other: both represent an ed25519 public key and traffic to either is encoded, encrypted, and verified the same way.  They are distinguished via the surrounding context in which they are encoded (see Apendix A: [carrier frame](#appendix-a-mrep-message-specification) & [manifest frame](#manifest-frame).

Routing of messages to a space address aligns closer to how announcements are spread through the network.

For any incoming message sent to a router first checks whether the given space address is locally registered and "up".  If true a copy of the message is either a) cached for later and/or b) forwarded to active subscriptions.  In either case the message is re-broadcast to all active neighbours (excluding the one the message was received from).  This procedure repeats for every router on the network until there are none left that haven't received the message.
...
- `recipient` :: (Optional) recipient address key.  May be replaced with a single zero byte if the frame is not addressed (see below).  When present a regular address is prefixed with a `1` and a space address with a `2`.
