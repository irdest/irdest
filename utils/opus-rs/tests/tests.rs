...
fn encode_mono() {
    let mut encoder =
        opus_rs::Encoder::new(48000, opus_rs::Channels::Mono, opus_rs::Application::Audio).unwrap();
    ...
    let myvec = encoder
        .encode_vec(&[1_i16; MONO_20MS], output.len())
        .unwrap();
...
#[test]
fn encode_stereo() {
    let mut encoder = opus_rs::Encoder::new(
        48000,
        opus_rs::Channels::Stereo,
    ...
    let len = encoder
        .encode(&[0_i16; 2 * MONO_20MS], &mut output)
        .unwrap();
    assert_eq!(&output[..len], &[252, 255, 254]);

    let len = encoder
        .encode(&[0_i16; 4 * MONO_20MS], &mut output)
        .unwrap();
    assert_eq!(&output[..len], &[253, 255, 254, 255, 254]);

    let len = encoder
        .encode(&[17_i16; 2 * MONO_20MS], &mut output)
        .unwrap();
    assert!(len > 240);
    ...
    let len = encoder
        .encode(&[95_i16; 2 * MONO_20MS], &mut [0; 20])
        .unwrap();
    assert!(len <= 20);
